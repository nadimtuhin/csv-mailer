...
export default function CampaignDetailPage() {
  ...
  const campaignId = params.campaignId as string; // Get ID from route
  ...
  // Fetch campaign details function
  ...
  const handleRetryFailed = async () => {
      ...
      try {
          // We can reuse the process endpoint, but ideally, it should only process 'failed' status on retry
          // Option 1: Modify process endpoint to accept a 'retry=true' flag or similar
          // Option 2: Create a new endpoint /api/campaigns/[campaignId]/retry
          // For simplicity, let's assume the process endpoint can handle retrying 'failed' ones (needs backend update)
          // Send retryFailed flag in the request body
          const response = await fetch(`/api/campaigns/${campaignId}/process`, {
              ...
              body: JSON.stringify({ retryFailed: true })     // Send the flag
          });
          ...
          setError(message); // Show error on this page
      ...
      <div className="w-full max-w-6xl">
        ...
        <section className="bg-white p-6 rounded-lg shadow mb-8">
           ...
           <div className="mt-6 flex gap-4">
                ...
                {/* Manual Process Trigger (if needed and not completed/failed) */}
